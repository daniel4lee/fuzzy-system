...
class PlotCanvas(FigureCanvas):
    ...
    def plot_map(self, imap):
        ...
        self.car_center = (self.dataset["{}".format(imap)].start[0], self.dataset["{}".format(imap)].start[1])
        #ini_car_center = [[self.dataset["{}".format(imap)].start[0], self.dataset["{}".format(imap)].start[1]], [fxpoint,fypoint]]

        # plot on figurecanvas
        self.ax.plot(*np.array(map_line).T, color='k', linewidth=3, solid_capstyle='round')
        self.ax.add_patch(descartes.PolygonPatch(end_area, fc='red', alpha=0.7))
        
        self.dir = self.ax.arrow(*self.car_center, *self.dir_point, head_width=1, head_length=1, fc='gold', ec='k')
        #self.dir = self.ax.plot(*np.array(inidir).T, color='navy', linewidth=1, solid_capstyle='round')
        self.car = self.ax.add_patch(descartes.PolygonPatch(inicar, fc='royalblue', alpha=0.5))
    ...
    def plot_car(self, list9d):
        ...
        for i in range(0, len(list9d[0])):
            ...
            right_line = self.ax.plot([right_point[0], list9d[7][i][0]],[right_point[1], list9d[7][i][1]], linestyle='--', color='navy')

            left_point = ( self.car_center[0] + 3*(math.cos(math.radians(list9d[9][i]+45))), 
            self.car_center[1] + 3*(math.sin( math.radians(list9d[9][i]+45) )) )
            left_line = self.ax.plot([left_point[0], list9d[8][i][0]],[left_point[1], list9d[8][i][1]], linestyle='--', color='navy')
            self.draw()
            front_line[0].remove()
            right_line[0].remove()
            left_line[0].remove()
